...
async function main() {
  ...
  let accessMode = apiKey ? 'authenticated_request' : 'public_low_volume_request';
  ...
  if (payload.status !== 'REQUEST_SUCCEEDED' && apiKey) {
    accessMode = 'public_low_volume_fallback';
  ...
  const snapshot = {
    ...
    accessMethod: accessMode === 'public_low_volume_fallback' ? 'BLS public API snapshot refreshed through the public low-volume path after the primary request was unavailable.' : 'BLS public API snapshot refreshed server-side; only public observations and provenance are written.',
