...
async function main() {
  ...
  const snapshot = {
    ...
    credentialStatus: {
      appToken: socrataCredentialStatus,
      apiSecret: hasSecret ? 'local_secret_configured_not_used_for_public_reads' : 'missing'
    },
    observedShape: {
  ...
  console.log(`Credential status: appToken=${snapshot.credentialStatus.appToken}, apiSecret=${snapshot.credentialStatus.apiSecret}`);
