...
async function main() {
  ...
  const snapshot = {
    ...
    accessMethod: socrataCredentialStatus.includes('fell_back_to_public_low_volume') ? 'CDC PLACES Socrata public API snapshot refreshed through the public low-volume path after the primary request was unavailable.' : 'CDC PLACES Socrata public API snapshot refreshed server-side; only public observations and provenance are written.',
